...
EOF
  ...
  for OPT in "${LEVELS[@]}"; do
    BC="$HOME/${NAME}_${OPT}.bc"
    OUT="$HOME/${NAME}_${OPT}.result"
    ...
    # --- compile to bit‑code ---
    "$CLANG" -c -emit-llvm "-${OPT}" \
             -fno-optimize-sibling-calls \
             "$SRC" -o "$BC"

    # pick first function in the module
    FUNC=$(llvm-dis -o - "$BC" |
           grep -m1 -oP 'define\s+.*?@(\w+)' |
           sed -E 's/.*@([A-Za-z0-9_]+).*/\1/')

    if [[ -z "$FUNC" ]]; then
      echo "[cirecmany] no function detected" > "$OUT"
      echo "⚠️   Skipped $BC — no function symbols" >&2
    ...
    if "$CIRE" "$BC" --function "$FUNC" --input "$INPUT_BIN" --debug-level 1 > "$OUT" 2>&1; then
      echo "✅  $OUT (opt ${OPT})"
